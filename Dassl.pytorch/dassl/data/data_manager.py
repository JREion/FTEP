...
class DataManager:
    ...
    def __init__(
        ...
        self.test_loader_fgpt = test_loader_fgpt
...
class DatasetWrapperFGPT(TorchDataset):
    ...
    def _path_parts(path):
        path = osp.normpath(str(path))
        drive, tail = osp.splitdrive(path)
        parts = [p for p in tail.split(osp.sep) if p and p != "."]
        if drive:
    ...
    def _dedupe(seq):
        ...
        out = []
        for item in seq:
            key = osp.normcase(osp.normpath(item))
            if key not in seen:
                seen.add(key)
                out.append(item)
        return out

    def _first_level_dirs(self, root):
        if not osp.isdir(root):
            return []
        try:
            return [
                name for name in os.listdir(root)
                if osp.isdir(osp.join(root, name))
            ]
        except OSError:
            return []

    def _relative_path_candidates(self, impath, root):
        candidates = []
        root_abs = osp.abspath(root)
        impath_abs = osp.abspath(impath)

        try:
            if osp.commonpath([root_abs, impath_abs]) == root_abs:
                rel_path = osp.relpath(impath_abs, root_abs)
                if not rel_path.startswith(".." + osp.sep) and rel_path != "..":
                    candidates.append(rel_path)
        except ValueError:
            pass

        parts = self._path_parts(impath)
        for anchor in self._first_level_dirs(root):
            anchor_lower = anchor.lower()
            for i, part in enumerate(parts):
                if part.lower() == anchor_lower:
                    tail = parts[i + 1:]
                    if tail:
                        candidates.append(osp.join(anchor, *tail))

        return self._dedupe(candidates)

    def _resolve_image_path(self, impath):
        """Prefer the current DATA tree when few-shot pkl files contain stale paths."""
        candidates = [osp.join(self.root, rel) for rel in self._relative_path_candidates(impath, self.root)]
    ...
    def _get_mask_path(self, impath):
        ...
        Few-shot pkl files can retain absolute paths from a previous checkout
        (for example D:\\Models\\DPC\\DATA\\...). Reconstruct mask paths from
        directory anchors under DATASET.ROOT/mask instead of trusting the raw
        absolute prefix.
        """
        candidates = [osp.join(self.mask_root, rel) for rel in self._relative_path_candidates(impath, self.mask_root)]
        expanded = []
        for path in candidates:
            expanded.append(path)
            try:
                rel = osp.relpath(path, self.mask_root)
            except ValueError:
                continue
            if rel.startswith(".." + osp.sep) or rel == "..":
                continue
            parts = [p for p in rel.split(osp.sep) if p and p != "."]
            if len(parts) >= 2 and parts[1].lower() != "sun397":
                expanded.append(osp.join(self.mask_root, parts[0], "SUN397", *parts[1:]))
        candidates = self._dedupe(expanded)
        for path in candidates:
            if osp.isfile(path):
                return path

        fallback = osp.join(self.mask_root, osp.relpath(impath, self.root))
        if candidates:
    ...
    def __getitem__(self, idx):
        ...
        impath = self._resolve_image_path(item.impath)
        output["impath"] = impath

        img0 = read_image(impath)        # PIL.Image (RGB)
        mask_path = self._get_mask_path(impath)
